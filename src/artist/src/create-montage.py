...
def create_montage_from_eeg(filename,chunk_size=10):
	...
	for current_chunk,chunk_i in enumerate(chunk_indices[0:-1]):
		...
		if (chunk_size < 1):
			seed = generate_seed(filtered_data,method='time-series')
		
		else:
			seed = generate_seed(filtered_data,method='power')		
	...
	imageio.mimwrite(file_full_path,image_data,format='.gif',fps=1)
...
if __name__ == "__main__":
	...
	create_montage_from_eeg(argv[1],float(argv[2]))
